package cn.bravedawn.rabbit.task.parser;

import cn.bravedawn.rabbit.task.annotation.ElasticJobConfig;
import cn.bravedawn.rabbit.task.autoconfigure.JobZookeeperProperties;
import cn.bravedawn.rabbit.task.enums.ElasticJobTypeEnum;
 ...
 * @description: 配置解析类
 * @date : Created in 2021/3/14 21:40
 */
@Slf4j
public class ElasticJobConfParser implements ApplicationListener<ApplicationReadyEvent> {
    ...
    public ElasticJobConfParser(JobZookeeperProperties zookeeperProperties, ZookeeperRegistryCenter zookeeperRegistryCenter) {
        ...
        try {
            ...
            for (Iterator<?> it = beanMap.values().iterator(); it.hasNext();){
                ...
                if (cls.getName().indexOf("$") > 0) {
                    ...
                    cls = Class.forName(className.substring(0, className.indexOf("$")));
                }

                // 获取接口类型，用于判断是什么类型的任务
                String jobTypeName = cls.getInterfaces()[0].getSimpleName();
                // 获取配置项 ElasticJobConfig
                ElasticJobConfig conf = cls.getAnnotation(ElasticJobConfig.class);
                ...
                // 4.1 添加bean构造参数，相当于添加自己的真实的任务实现类
                if (!ElasticJobTypeEnum.SCRIPT.equals(jobTypeName)) {
                ...
                // 4.3 添加liteJobConfiguration
                factory.addConstructorArgValue(liteJobConfiguration);

                // 4.4 如果有eventTraceRdbDataSource，则也进行添加
                ...
                // 	5 接下来就是把factory 也就是 SpringJobScheduler注入到Spring容器中
